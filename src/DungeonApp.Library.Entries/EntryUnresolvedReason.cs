...
/// entirely on the other side of the pack-to-system boundary - no system answers to the
/// id this entry names, the system that does answer to it does not declare this type, the
/// type it declares exists only at a different version, or the system knows the type but
/// refused the values this entry supplied.
/// </summary>
public enum EntryUnresolvedReason
{
    /// <summary>
    /// No installed system answers to this entry's content type reference at all - see
    /// <see cref="IContentTypeCatalog.HasSet"/>.
    /// </summary>
    MissingSet,

    /// <summary>
    /// The system this entry's content type reference names is installed, but that set
    ...
    /// <summary>
    /// The system knows this content type and version, but refused the entry's values -
