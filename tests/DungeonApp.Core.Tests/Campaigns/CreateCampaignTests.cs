...
public sealed class CreateCampaignTests
{
    ...
    private static readonly SystemId TestSystemId = SystemId.Create("test-system");
    ...
    {
        var created = await _createCampaign.ExecuteAsync("Kroniki Doliny", TestSystemId, []);
    ...
    public async Task Stamps_the_campaign_with_the_active_system()
    {
        ...
        Assert.Equal(TestSystemId, created.SystemId);
    }
    ...
    {
        var first = await _createCampaign.ExecuteAsync("Kroniki Doliny", TestSystemId, []);
        var second = await _createCampaign.ExecuteAsync("Kroniki Doliny", TestSystemId, []);
    ...
    {
        await Assert.ThrowsAsync<ArgumentException>(() => _createCampaign.ExecuteAsync("   ", TestSystemId, []));
